...
WORKING_DIRECTORY_BASE_NAME = f"gemini_{datetime.now().strftime('%d.%m.%Y-%H:%M:%S')}"
...
df = pd.read_csv("/u1/m2fetrat/GhCodes/visual-reasoning/result_final/emoji_benchmark_one_canvas_ours/output.csv")
# Create the model
...
for _, row in tqdm(df.iterrows(), total=len(df)):
    ...
    total_count = 0
    for image_path in sorted(glob.glob(f"/u1/m2fetrat/GhCodes/visual-reasoning/result_final/emoji_benchmark_one_canvas_ours/{folder_name}/**/**_subimages/*.png", recursive=True)):
        while True:
            try:
                files = [
