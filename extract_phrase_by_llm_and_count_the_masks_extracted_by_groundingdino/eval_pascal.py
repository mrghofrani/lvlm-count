...
parser.add_argument('--image_base_path', type=str, required=False)
...
def main():
    ...
    for index, row in tqdm(df.iterrows(), total=len(df)):
        logger.debug("object of interest: %s", row['category_name'])
        ...
        working_dir = f"{WORKING_DIRECTORY_BASE_NAME}/{index}.{row['category_name']}"
        os.mkdir(working_dir)
        logger.debug("output directory: %s", working_dir)
        count = method_wrapper(row["category_name"], image_path, working_dir)
        out_df.append(
            row.tolist()
            + [
                count,
