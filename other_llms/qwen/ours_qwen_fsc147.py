...
WORKING_DIRECTORY_BASE_NAME = f"ours_qwen_fsc147_{datetime.now().strftime('%d.%m.%Y-%H:%M:%S')}"
...
for i in range(5):
    ...
    for _, row in tqdm(df.iterrows(), total=len(df)):
        ...
        for image_path in sorted(glob.glob(f"output/{image_name}/**/**_subimages/*.png", recursive=True)):
            prompt = f"Count (if not possible, Estimate) the number of visible {row['object_of_interest']} in the image. If you don't see any say zero. Report your final answer with digits inside **BOLDS**."
