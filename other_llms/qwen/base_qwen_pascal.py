...
WORKING_DIRECTORY_BASE_NAME = f"base_pascal_qwen_{datetime.now().strftime('%d.%m.%Y-%H:%M:%S')}"
os.mkdir(WORKING_DIRECTORY_BASE_NAME)


df = pd.read_csv("data/PASCAL/sampled_pascal.csv")
...
for i in range(3):
    ...
    for index, row in tqdm(df.iterrows(), total=len(df)):
        ...
        total_count = 0
        image_path = f"data/PASCAL/VOCdevkit/VOC2007/JPEGImages/{image_name}"
        prompt = f"Count (if not possible, Estimate) the number of visible {row['category_name']} in the image. Report your answer with digit inside **BOLDS**."
