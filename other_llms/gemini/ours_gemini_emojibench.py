...
WORKING_DIRECTORY_BASE_NAME = (
    f"ours_gemini_emojibench_{datetime.now().strftime('%d.%m.%Y-%H:%M:%S')}"
)
...
for i in range(2):
    ...
    formatter = logging.Formatter(
        "%(asctime)s %(name)s %(msecs)d %(levelname)s fn:%(funcName)s -- %(message)s"
    )
    # Adding file handler
    fh = logging.FileHandler(
        f"{WORKING_DIRECTORY_BASE_NAME}/log{i}.log", encoding="utf-8"
    )
    ...
    for _, row in tqdm(df.iterrows(), total=len(df)):
        ...
        for image_path in sorted(
            glob.glob(
                f"output/{folder_name}/**/**_subimages/*.png",
                recursive=True,
            )
        ):
            ...
            while True:
                try:
                    ...
                    response = json.loads(
                        chat_session.send_message([files[0], prompt]).text
                    )
                    break
                except Exception:
        ...
        logger.info(f"total count for %s was %d", row["image_file"], total_count)
        out_df.append(
            (
                row["id"],
                row["class"],
                row["image_file"],
                row["object_of_interest"],
                row["answer"],
                total_count,
            )
        )
        ...
        ).to_csv(f"{WORKING_DIRECTORY_BASE_NAME}/output{i}.csv", index=False)
