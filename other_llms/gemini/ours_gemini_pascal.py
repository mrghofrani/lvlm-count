...
WORKING_DIRECTORY_BASE_NAME = f"ours_gemini_pascal_{datetime.now().strftime('%d.%m.%Y-%H:%M:%S')}"
...
for i in range(3):
    ...
    for index, row in tqdm(df.iterrows(), total=len(df)):
        ...
        for image_path in sorted(glob.glob(f"output/{image_name}/**/**_subimages/*.png", recursive=True)):
            prompt = f"How many {row['category_name']} are visibile in the image?"
        ...
        pd.DataFrame(out_df, columns=["id", "image", "category_name", "answer", "llm_count"]).to_csv(f"{WORKING_DIRECTORY_BASE_NAME}/output{i}.csv", index=False)
